...
sorted_grouped_by_category_question["Sensitive Information"].append(137)
...
# ## Calculate JSR - GPT3.5
jsr_detail_df = data_df_35.groupby(["prompt_category", "question_category"]).agg(
    mean=('mean', 'mean'),
    std=('mean', 'std')
).reset_index()

overall_prompt_jsr = data_df_35.groupby("prompt_category").agg(
    mean=('mean', 'mean'),
    std=('mean', 'std')
).reset_index()
overall_prompt_jsr["question_category"] = "Overall"

jsr_detail_df_with_overall = pd.concat([jsr_detail_df, overall_prompt_jsr], ignore_index=True)

pivoted_jsr_mean_df_with_overall = jsr_detail_df_with_overall.pivot(index="question_category", columns="prompt_category", values="mean")
pivoted_jsr_std_df_with_overall = jsr_detail_df_with_overall.pivot(index="question_category", columns="prompt_category", values="std")

# Save to Excel
with pd.ExcelWriter("JSR_GPT35.xlsx") as writer:
...
# ## Calculate JSR - GPT4
jsr_detail_df = data_df_4.groupby(["prompt_category", "question_category"]).agg(
    mean=('mean', 'mean'),
    std=('mean', 'std')
).reset_index()

overall_prompt_jsr = data_df_4.groupby("prompt_category").agg(
    mean=('mean', 'mean'),
    std=('mean', 'std')
).reset_index()
overall_prompt_jsr["question_category"] = "Overall"
...
# Save to Excel
with pd.ExcelWriter("JSR_GPT4.xlsx") as writer:
    pivoted_jsr_mean_df_with_overall.to_excel(writer, sheet_name="Mean")
    pivoted_jsr_std_df_with_overall.to_excel(writer, sheet_name="Std")


# ## Calculate JSR - Palm
jsr_detail_df = data_df_Palm.groupby(["prompt_category", "question_category"]).agg(
    mean=('mean', 'mean'),
    std=('mean', 'std')
).reset_index()

overall_prompt_jsr = data_df_Palm.groupby("prompt_category").agg(
    mean=('mean', 'mean'),
    std=('mean', 'std')
).reset_index()
overall_prompt_jsr["question_category"] = "Overall"
...
with pd.ExcelWriter("JSR_PaLM2.xlsx") as writer:
    pivoted_jsr_mean_df_with_overall.to_excel(writer, sheet_name="Mean")
    pivoted_jsr_std_df_with_overall.to_excel(writer, sheet_name="Std")


# ## Calculate EMH - GPT3.5
emh_df = data_df_35.groupby(["prompt_category", "question_category"]).agg(
    mean=('max', 'mean'),
    std=('max', 'std')
).reset_index()

overall_prompt_emh = data_df_35.groupby("prompt_category").agg(
    mean=('max', 'mean'),
    std=('max', 'std')
).reset_index()
overall_prompt_emh["question_category"] = "Overall"

emh_df_with_overall = pd.concat([emh_df, overall_prompt_emh], ignore_index=True)

pivoted_emh_mean_df_with_overall = emh_df_with_overall.pivot(index="question_category", columns="prompt_category", values="mean")
pivoted_emh_std_df_with_overall = emh_df_with_overall.pivot(index="question_category", columns="prompt_category", values="std")

output_path_emh_updated_with_overall = "EMH_GPT35.xlsx"
with pd.ExcelWriter(output_path_emh_updated_with_overall) as writer:
...
# ## Calculate EMH - GPT4
emh_df = data_df_4.groupby(["prompt_category", "question_category"]).agg(
    mean=('max', 'mean'),
    std=('max', 'std')
).reset_index()

overall_prompt_emh = data_df_4.groupby("prompt_category").agg(
    mean=('max', 'mean'),
    std=('max', 'std')
).reset_index()
overall_prompt_emh["question_category"] = "Overall"
...
output_path_emh_updated_with_overall = "EMH_GPT4.xlsx"
with pd.ExcelWriter(output_path_emh_updated_with_overall) as writer:
    pivoted_emh_mean_df_with_overall.to_excel(writer, sheet_name="Mean")
    pivoted_emh_std_df_with_overall.to_excel(writer, sheet_name="Std")
...
# ## Calculate EMH - Palm
emh_df = data_df_Palm.groupby(["prompt_category", "question_category"]).agg(
    mean=('max', 'mean'),
    std=('max', 'std')
).reset_index()

overall_prompt_emh = data_df_Palm.groupby("prompt_category").agg(
    mean=('max', 'mean'),
    std=('max', 'std')
).reset_index()
overall_prompt_emh["question_category"] = "Overall"
...
output_path_emh_updated_with_overall = "EMH_PaLM2.xlsx"
with pd.ExcelWriter(output_path_emh_updated_with_overall) as writer:
    pivoted_emh_mean_df_with_overall.to_excel(writer, sheet_name="Mean")
    pivoted_emh_std_df_with_overall.to_excel(writer, sheet_name="Std")
